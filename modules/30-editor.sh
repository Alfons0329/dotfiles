...
write_theme_marker() {
    local marker="$HOME/.dotfiles_theme" theme="${DOTFILES_THEME:-tokyonight}"

    if [ "$DRY_RUN" = "1" ]; then
        run_sh "cat > '$marker' <<'EOF' (editor/terminal theme: $theme)"
        return
    fi
...
# Re-run install.sh with --theme to change. Choices: tokyonight (default), kanagawa, ayu-dark.
